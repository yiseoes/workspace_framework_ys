...
<div class="panel">
  ...
  <c:choose>
    ...
    <c:otherwise>
      <div class="prod">
        <img src="/images/${product.imageFile}" alt="${fn:escapeXml(product.prodName)}">
        <div>
      ...
      <form action="<c:url value='/purchase/addPurchase'/>" method="post" style="margin-top:14px">
        ...
        <div class="row">
          ...
          <div>
            <c:choose>
              <c:when test="${not empty login}">${login.userId}</c:when>
        ...
        <div class="actions">
          ...
          <a class="btn" href="<c:url value='/product/getProduct'><c:param name='prodNo' value='${product.prodNo}'/></c:url>">상품 상세</a>
          <a class="btn" href="<c:url value='/product/listProduct'/>">목록</a>
        </div>
      </form>
    </c:otherwise>
  </c:choose>
