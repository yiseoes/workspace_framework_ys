...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:13px;">
  ...
  <tr><td height="1" colspan="3" bgcolor="D6D6D6"></td></tr>
  <tr>
    <td width="104" class="ct_write">거래상태</td>
    <td bgcolor="D6D6D6" width="1"></td>
    <td class="ct_write01">
      <c:choose>
        <c:when test="${empty product.proTranCode}">-</c:when>
        <c:otherwise>${product.proTranCode}</c:otherwise>
      </c:choose>
    </td>
  </tr>
</table>
...
<div style="margin-top:10px; text-align:right;">
  <form action="/listProduct.do" method="get" style="display:inline;">
    <input type="hidden" name="menu" value="${param.menu}" />
    <button type="submit">목록</button>
  </form>

  <!-- 구매하기 : 상품상세 → 구매작성 화면 이동 -->
	<!-- 구매하기 : 컨트롤러(addPurchaseView.do)로만 이동 -->
	<form action="/addPurchaseView.do" method="get" style="display:inline; margin-left:8px;">
	  <input type="hidden" name="prodNo" value="${product.prodNo}" />
	  <button type="submit">구매하기</button>
	</form>

  <c:if test="${isAdmin}">
    <form action="/updateProductView.do" method="get" style="display:inline; margin-left:8px;">
      <input type="hidden" name="prodNo" value="${product.prodNo}" />
