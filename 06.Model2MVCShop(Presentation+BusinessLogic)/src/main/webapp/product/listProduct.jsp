...
<form name="detailForm" action="/listProduct.do" method="get">
  ...
  <table width="100%" height="37" border="0" cellpadding="0" cellspacing="0">
    <tr>
      ...
      <td width="14" height="37"><img src="/images/ct_ttl_img03.gif" width="14" height="37" /></td>
    </tr>
  </table>

  <!-- Search : 모든 사용자 노출(유저 UI와 동일 스타일) -->
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:10px;">
    ...
    <c:otherwise>
      ...
      <c:forEach var="p" items="${list}">
        ...
        <tr class="ct_list_pop">
          ...
          <td align="left">
            <a href="/getProduct.do?prodNo=${p.prodNo}">${p.prodName}</a>
          </td>
          <td></td>
          ...
          <td align="right">
            <c:choose>
              ...
              <c:otherwise><fmt:formatNumber value="${p.price}" type="number" /></c:otherwise>
            </c:choose>
          </td>
          <td></td>
          ...
          <td align="center">
            <c:choose>
              ...
              <c:otherwise><fmt:formatDate value="${p.regDate}" pattern="yyyy-MM-dd" /></c:otherwise>
            </c:choose>
          </td>
          <td></td>
          ...
          <td align="center">
            <c:choose>
              <c:when test="${p.proTranCode == 'SOLD'}">판매완료</c:when>
  ...
  <!-- 등록 버튼 : 관리자 + manage 흐름일 때만, 폼 맨 아래에 1회 노출 -->
  <c:if test="${isAdmin and isManage}">
    <div style="margin-top:12px; text-align:right;">
      <button type="submit" formaction="/addProductView.do" formmethod="get">등록</button>
    </div>
  </c:if>
