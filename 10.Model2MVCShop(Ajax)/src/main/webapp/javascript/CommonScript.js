/* ===== 기존 CommonScript.js 원본 (네가 쓰던 검증 로직 그대로) ===== */
...
/* ====== 추가: alert를 #formError로 라우팅 (팝업 대신 화면에 표시) ====== */
(function () {
  if (window.__alertHookInstalled) return;
  window.__alertHookInstalled = true;

  var nativeAlert = window.alert;
  window.alert = function (msg) {
    try {
      var $box = window.jQuery ? jQuery("#formError") : null;
      if ($box && $box.length) {
        $box.text(msg || "");
      } else {
...
/* ====== 추가: (로그인 필드 한정) 영문 검증 메시지 → 한글 치환 ====== */
(function () {
  if (window.__loginMsgHookInstalled) return;
  window.__loginMsgHookInstalled = true;

  var routedAlert = window.alert;

  function ko(msg) {
    var s = String(msg || "");

    // 필수 입력
    s = s.replace(/^아이디\s+is a required field\.\s*$/i, "아이디를 입력해 주세요.");
    s = s.replace(/^비밀번호\s+is a required field\.\s*$/i, "비밀번호를 입력해 주세요.");

    // 최소/최대 길이
    s = s.replace(/^(아이디|비밀번호)\s+has under min length.*:\s*(\d+)\s*$/i,
                  "$1는 최소 $2자 이상 입력해 주세요.");
    s = s.replace(/^(아이디|비밀번호)\s+has over max length.*:\s*(\d+)\s*$/i,
                  "$1는 최대 $2자를 초과할 수 없습니다.");

    // 형식 오류
    s = s.replace(/^(아이디|비밀번호)\s+has invalid format\..*$/i,
                  "$1 형식이 올바르지 않습니다.");

    // 특수문자 금지
    s = s.replace(/^You can't enter special characters.*in\s+(아이디|비밀번호)\s+field\.\s*$/i,
                  "$1에는 특수문자를 입력할 수 없습니다.");

    return s;
  }

  window.alert = function (msg) {
    return routedAlert(ko(msg));
  };
})();
